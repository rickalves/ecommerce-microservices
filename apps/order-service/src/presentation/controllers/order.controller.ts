    ...
    private async processEvent(
        ...
        try {
            ...
            if (retries >= MAX_RETRIES) {
                console.error(`[OrderService] Handler '${pattern}' falhou após ${MAX_RETRIES} tentativas. Enviando para DLQ.`, {
                    pattern,
                    error: error.message,
                    retries,
                });
                ...
                console.warn(`[OrderService] Handler '${pattern}' falhou. Agendando retry (${retries + 1}/${MAX_RETRIES}).`, {
                    pattern,
                    error: error.message,
                });
