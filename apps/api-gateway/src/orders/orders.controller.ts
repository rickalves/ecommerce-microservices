...
export class OrdersController {
  ...
  async getAllOrders() {
    try {
      return await firstValueFrom(
        this.orderService.send({ cmd: 'get_all_orders' }, {}),
      );
    } catch (error) {
      throw new HttpException(
        error.message || 'Failed to fetch orders',
  ...
  async confirmOrder(@Param('id') id: string) {
    try {
      return await firstValueFrom(
        this.orderService.send({ cmd: 'confirm_order' }, id),
      );
    } catch (error) {
      throw new HttpException(
  ...
  async shipOrder(@Param('id') id: string) {
    try {
      return await firstValueFrom(
        this.orderService.send({ cmd: 'ship_order' }, id),
      );
    } catch (error) {
      throw new HttpException(
  ...
  async deliverOrder(@Param('id') id: string) {
    try {
      return await firstValueFrom(
        this.orderService.send({ cmd: 'deliver_order' }, id),
      );
    } catch (error) {
      throw new HttpException(
  ...
  async cancelOrder(@Param('id') id: string) {
    try {
      return await firstValueFrom(
        this.orderService.send({ cmd: 'cancel_order' }, id),
      );
    } catch (error) {
      throw new HttpException(
